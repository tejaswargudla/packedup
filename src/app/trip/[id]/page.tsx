...
import { tripsApi, suggestionsApi } from '@/lib/api'
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <header style={{ height: '56px', background: 'rgba(8,14,26,0.97)', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'stretch', position: 'sticky', top: 0, zIndex: 50, backdropFilter: 'blur(12px)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', borderLeft: '1px solid var(--border)' }}>
          ...
          <button onClick={() => router.push(`/trip/${trip.id}/created`)} style={{ height: '100%', padding: '0 24px', fontFamily: 'var(--font-mono)', fontSize: '10px', letterSpacing: '3px', textTransform: 'uppercase', color: 'var(--bg)', background: 'var(--gold)', border: 'none', cursor: 'pointer' }}>
            + Invite
          </button>
        </div>
      </header>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <main style={{ flex: 1, overflowY: 'auto' }}>
          ...
          {/* PAGE HEADER */}
          <div style={{ padding: '48px 56px 36px', borderBottom: '1px solid var(--border)', background: 'rgba(8,14,26,0.6)', backdropFilter: 'blur(8px)' }}>
            ...
            <h1 style={{ fontSize: 'clamp(32px,4vw,52px)', fontWeight: 300, letterSpacing: '-1px', lineHeight: 0.95, marginBottom: '10px' }}>
              {trip.name} <em style={{ color: 'var(--gold)' }}>is coming together.</em>
            </h1>
            <p style={{ fontSize: '14px', fontWeight: 300, fontStyle: 'italic', color: 'var(--muted2)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', borderBottom: '1px solid var(--border)' }}>
            {[
              ...
              <div key={i} style={{ padding: '28px 32px', borderRight: i < 3 ? '1px solid var(--border)' : 'none', transition: 'background 0.2s' }}
                ...
                <div style={{ fontSize: 'clamp(36px,4vw,52px)', fontWeight: 300, lineHeight: 1, letterSpacing: '-1px', color: 'var(--gold)' }}>{s.n}</div>
                <div style={{ fontSize: '12px', fontWeight: 300, color: 'rgba(232,238,248,0.35)', marginTop: '4px' }}>{s.lbl}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', flex: 1 }}>
            <div style={{ borderRight: '1px solid var(--border)' }}>
              ...
              {boards.map(b => {
                ...
                return (
                  <motion.div key={b.type} whileHover={{ paddingLeft: '48px' }} style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '20px 32px', borderBottom: '1px solid var(--border)', cursor: 'pointer', transition: 'background 0.2s' }}
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: '16px', fontWeight: 300, marginBottom: '3px' }}>{b.label}</div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '20px 32px', cursor: 'pointer', borderTop: '1px solid var(--border)' }}
                ...
                onMouseEnter={e => (e.currentTarget.style.background = 'var(--surface)')}
                onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                <div style={{ width: '40px', height: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', border: '1px solid rgba(201,168,76,0.3)' }}>🗓️</div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '16px', fontWeight: 300, marginBottom: '3px' }}>View Itinerary</div>
            ...
            <div>
              ...
              {members.map((m, i) => (
                <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '16px 24px', borderBottom: '1px solid var(--border)', transition: 'background 0.2s' }}
                  ...
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', letterSpacing: '2px', textTransform: 'uppercase', padding: '3px 10px', border: `1px solid ${m.role === 'creator' ? 'rgba(201,168,76,0.4)' : 'var(--border2)'}`, color: m.role === 'creator' ? 'var(--gold)' : 'var(--muted2)', background: m.role === 'creator' ? 'rgba(201,168,76,0.06)' : 'transparent' }}>
                    {m.role}
                  </div>
                </div>
              ))}
