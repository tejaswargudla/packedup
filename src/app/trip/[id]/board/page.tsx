...
export default function BoardPage({ params }: { params: { id: string } }) {
  ...
  return (
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* TOPBAR */}
      <header style={{ height: '56px', background: 'rgba(8,14,26,0.97)', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'stretch', position: 'sticky', top: 0, zIndex: 50, backdropFilter: 'blur(12px)' }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: '16px', fontWeight: 300, letterSpacing: '6px', textTransform: 'uppercase', color: 'var(--gold)', padding: '0 28px', borderRight: '1px solid var(--border)', display: 'flex', alignItems: 'center' }}>PackedUp</div>
        ...
        <div style={{ display: 'flex', alignItems: 'stretch' }}>
          ...
          <button onClick={() => router.push(`/trip/${params.id}/created`)} style={{ height: '100%', padding: '0 24px', fontFamily: 'var(--font-mono)', fontSize: '10px', letterSpacing: '3px', textTransform: 'uppercase', color: 'var(--bg)', background: 'var(--gold)', border: 'none', cursor: 'pointer' }}>
            Invite
          </button>
        </div>
      </header>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        {/* SIDEBAR */}
        <aside style={{ width: '220px', flexShrink: 0, background: 'rgba(8,14,26,0.95)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', backdropFilter: 'blur(12px)' }}>
        ...
        <main style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
          ...
          {/* BOARD TAB NAV */}
          <div style={{ borderBottom: '1px solid var(--border)', display: 'flex', background: 'rgba(8,14,26,0.6)', backdropFilter: 'blur(8px)', flexShrink: 0 }}>
            {BOARDS.map(b => {
          ...
          <AnimatePresence>
            {finalized && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                style={{ background: 'rgba(201,168,76,0.08)', borderBottom: '1px solid rgba(201,168,76,0.2)', padding: '14px 32px', display: 'flex', alignItems: 'center', gap: '16px', flexShrink: 0 }}>
          ...
          <div style={{ padding: '32px 40px', display: 'flex', flexDirection: 'column', gap: '16px', flex: 1 }}>
            ...
            {!isLoading && suggestions.length === 0 && (
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} style={{ textAlign: 'center', padding: '80px 0' }}>
                ...
                <h2 style={{ fontSize: '28px', fontWeight: 300, letterSpacing: '-0.5px', marginBottom: '8px' }}>No suggestions yet</h2>
                ...
                <button onClick={() => setModalOpen(true)} style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', letterSpacing: '3px', textTransform: 'uppercase', padding: '14px 36px', background: 'var(--gold)', color: 'var(--bg)', border: 'none', cursor: 'pointer' }}>
                  + Add Suggestion
                </button>
              </motion.div>
            )}
              ...
              {suggestions.map((sug, i) => {
                ...
                return (
                  <motion.div key={sug.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                    style={{ border: `1px solid ${sug.is_finalized ? 'rgba(201,168,76,0.35)' : isLeading ? 'rgba(201,168,76,0.2)' : 'var(--border)'}`, background: sug.is_finalized ? 'rgba(201,168,76,0.04)' : 'var(--surface)', padding: '20px 24px', display: 'flex', gap: '20px', alignItems: 'flex-start' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', marginBottom: '6px', flexWrap: 'wrap' }}>
                        <span style={{ fontSize: '17px', fontWeight: 300, letterSpacing: '-0.3px' }}>{sug.name}</span>
                        {sug.is_finalized && (
                          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '8px', letterSpacing: '2px', textTransform: 'uppercase', padding: '3px 10px', background: 'rgba(201,168,76,0.15)', color: 'var(--gold)', border: '1px solid rgba(201,168,76,0.3)', flexShrink: 0 }}>
                            ✓ Finalized
                          </span>
                        )}
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                        ...
                        {isCreator && !sug.is_finalized && (
                          <button onClick={() => finalizeSug(sug.id)} disabled={finalizePending} style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', letterSpacing: '2px', textTransform: 'uppercase', padding: '6px 14px', background: 'transparent', border: '1px solid rgba(201,168,76,0.3)', color: 'var(--gold)', cursor: 'pointer', transition: 'all 0.15s' }}
            ...
            {suggestions.length > 0 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} onClick={() => setModalOpen(true)}
                ...
                onMouseEnter={e => { const el = e.currentTarget as HTMLDivElement; el.style.borderColor = 'rgba(201,168,76,0.4)'; el.style.color = 'var(--gold)' }}
